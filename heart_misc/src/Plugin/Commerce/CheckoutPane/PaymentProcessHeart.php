 ...
 *   id = "payment_process_heart",
 *   label = @Translation("Payment process Heart"),
 *   default_step = "payment",
 *   wrapper_element = "container",
 * )
 */
class PaymentProcessHeart extends CheckoutPaneBase {
  ...
  public function isVisible() {
    ...
    $payment_info_pane = $this->checkoutFlow->getPane('payment_information_custom');
  ...
  public function buildPaneForm(array $pane_form, FormStateInterface $form_state, array &$complete_form) {
    ...
    if ($payment_gateway_plugin instanceof SupportsStoredPaymentMethodsInterface && !$this->order->get('payment_method')->isEmpty()) {
      try {
        ...
        if (($route_name == 'commerce_checkout.form') && ($path_args[3] == 'payment')) {
          $path = \Drupal::service('path.current')->getPath();
          $path_args = explode('/', $path);
          $orderId = $path_args[2];
          $store = \Drupal::service('tempstore.private')->get('kh_product');
          if (!empty($store->get('locker' . $orderId . ''))) {
            $store->delete('locker' . $orderId . '');
          }
        }
  ...
  protected function getErrorStepId() {
    ...
    $step_id = $this->checkoutFlow->getPane('payment_information_custom')->getStepId();
    if ($step_id == '_disabled') {
      ...
      throw new \RuntimeException('Cannot get the step ID for the payment_information_custom pane. The pane is disabled.');
    }
