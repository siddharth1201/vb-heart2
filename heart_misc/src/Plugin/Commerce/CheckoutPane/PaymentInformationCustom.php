 ...
 *   id = "payment_information_custom",
 *   label = @Translation("Payment Information"),
 *   default_step = "order_information",
 *   wrapper_element = "fieldset",
 * )
 */
class PaymentInformationCustom extends CheckoutPaneBase {
  ...
  public function buildPaneForm(array $pane_form, FormStateInterface $form_state, array &$complete_form) {
    ...
    // Editing payment methods at checkout is nooptionst supported.
    ...
    $pane_form['view_heading'] = [
      '#type' => 'markup',
      '#markup' => $this->t('<span class="payment-info-text">* A receipt for this purchase will be sent to your email.</span>'),
    ];
    return $pane_form;
  }
  ...
  public function submitPaneForm(array &$pane_form, FormStateInterface $form_state, array &$complete_form) {
    ...
    $selected_option = !empty($pane_form['#payment_options'][$values['payment_method']]) ? $pane_form['#payment_options'][$values['payment_method']] : [];
    /** @var \Drupal\commerce_payment\PaymentGatewayStorageInterface $payment_gateway_storage */
    $payment_gateway_storage = $this->entityTypeManager->getStorage('commerce_payment_gateway');
    /** @var \Drupal\commerce_payment\Entity\PaymentGatewayInterface $payment_gateway */
    $payment_gateway = !empty($selected_option) ? $payment_gateway_storage->load($selected_option->getPaymentGatewayId()) : '';
    if (!$payment_gateway) {
      return;
    }
